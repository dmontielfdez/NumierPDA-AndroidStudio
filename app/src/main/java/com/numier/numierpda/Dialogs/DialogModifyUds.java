...
public class DialogModifyUds extends DialogFragment implements View.OnClickListener {
    ...
    public DialogModifyUds(Detalle detail, int position) {
        this.detail = detail;
    ...
    public void onStart() {
        ...
        if (d != null) {
            ...
            d.setTitle(getString(R.string.specify_uds_product));
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.buttonOkDialogKeyboard:
                ...
                double value = Double.parseDouble(countInteger.toString());

                if(value == 0){
                    Snackbar.make(v, R.string.uds_not_zero, Snackbar.LENGTH_LONG).show();
                } else{
                    this.detail.setQuantity(value);

                    Cash.getTicketFragment().getDetails().get(position).setQuantity(value);
                    Cash.getTicketFragment().getDetails().get(position).setAmount(Cash.getTicketFragment().getDetails().get(position).getQuantity() * (Cash.getTicketFragment().getDetails().get(position).getPrice()));
                    Cash.getTicketFragment().getAdapterList().notifyDataSetChanged();

                    ((Cash) getActivity()).getCashFragment().setDetailsToHeader(Cash.getTicketFragment().getDetails(), detail);
                    getDialog().cancel();
