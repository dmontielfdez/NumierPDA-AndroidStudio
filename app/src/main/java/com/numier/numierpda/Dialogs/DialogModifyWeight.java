...
public class DialogModifyWeight extends DialogFragment implements View.OnClickListener {
    ...
    public DialogModifyWeight(Detalle detail, int position) {
        this.detail = detail;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.buttonOkDialogKeyboard:
                ...
                if(value / 1000 == 0.0){
                    Snackbar.make(v, R.string.weight_not_zero, Snackbar.LENGTH_LONG).show();
                } else{
                    this.detail.setQuantity(value / 1000);

                    Cash.getTicketFragment().getDetails().get(position).setQuantity(value / 1000);
